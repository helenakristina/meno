...
export const providerSearchSchema = z
	.object({
		...
		city: z.string().max(100, 'City name must be under 100 characters').optional(),
		zip_code: z
			.string()
...
export const updateShortlistSchema = z.object({
	provider_id: z.string().uuid('Invalid provider ID'),
	status: z.enum(['contact_pending', 'contacted', 'completed', 'other']).describe('Call status'),
...
export const callingScriptSchema = z.object({
	provider_id: z.string().uuid('Invalid provider ID'),
	questions: z.array(z.string()).default([]).describe('Custom questions to include in script')
});
