...
<div class="mx-auto max-w-lg">
	...
	<h1 class="mb-6 text-2xl font-bold text-neutral-800">Add Medication</h1>

	{#if error}
		<div class="mb-4 rounded-md bg-danger-light p-4 text-sm text-danger-dark" role="alert">
			{error}
		</div>
	{/if}
	...
	<form onsubmit={handleSubmit} class="space-y-5" novalidate>
		...
		<div>
			<label for="medication_name" class="mb-1.5 block text-sm font-medium text-neutral-700">
				Medication name <span class="text-danger" aria-hidden="true">*</span>
			</label>
			<div class="relative">
				<input
					id="medication_name"
				...
				{#if showDropdown && (searchResults.length > 0 || searchQuery.trim().length >= 2)}
					<ul
						...
						class="absolute z-10 mt-1 max-h-64 w-full overflow-auto rounded-lg border border-neutral-200 bg-white shadow-md"
					>
						{#each searchResults as med, i}
							...
							>
								<span class="font-medium text-neutral-800">{med.generic_name}</span>
								{#if med.brand_name}
									<span class="text-neutral-500"> ({med.brand_name})</span>
								{/if}
								{#if med.common_forms.length > 0}
									<span class="ml-1.5 text-xs text-neutral-400"
						...
						<li
							...
							class="cursor-pointer border-t border-neutral-100 px-3 py-2.5 text-sm text-neutral-500 hover:bg-neutral-50 {highlightedIndex ===
		...
		<!-- Dose -->
		<div>
			<label for="dose" class="mb-1.5 block text-sm font-medium text-neutral-700">
				Dose <span class="text-danger" aria-hidden="true">*</span>
			</label>
		...
		<!-- Delivery method -->
		<div>
			<label for="delivery_method" class="mb-1.5 block text-sm font-medium text-neutral-700">
		...
		<!-- Frequency (optional) -->
		<div>
			<label for="frequency" class="mb-1.5 block text-sm font-medium text-neutral-700">
				Frequency <span class="text-xs font-normal text-neutral-400">(optional)</span>
			</label>
			<input
				id="frequency"
				type="text"
		...
		<!-- Start date -->
		<div>
			<label for="start_date" class="mb-1.5 block text-sm font-medium text-neutral-700">
		...
		<div>
			<label for="notes" class="mb-1.5 block text-sm font-medium text-neutral-700">
				Notes <span class="text-xs font-normal text-neutral-400">(optional)</span>
			</label>
			<textarea
				id="notes"
				bind:value={notes}
		...
		<div class="flex gap-3 pt-2">
			<a
				href="/medications"
				class="flex-1 rounded-lg border border-neutral-300 px-4 py-3 text-center text-sm font-medium text-neutral-700 hover:bg-neutral-50"
			>
				Cancel
			</a>
			<button
				...
				class="flex-1 rounded-lg bg-primary-500 px-4 py-3 text-sm font-medium text-white hover:bg-primary-600 disabled:cursor-not-allowed disabled:opacity-40"
			>
