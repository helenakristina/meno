...
- LLM prompt assembly and LLM API calls
- Anonymization layer before any data reaches LLM
...
LLM and Python have a clear division of labor:
...
| Write narrative from calculated findings | LLM         | Meaning-making, nuance         |
| Connect patterns to research context     | LLM         | RAG-grounded reasoning         |
| Analyze free text for semantic themes    | LLM         | Natural language understanding |
| Generate provider calling script         | LLM         | Natural language generation    |
| Generate doctor visit questions          | LLM         | Personalized reasoning         |
| Answer educational questions             | LLM         | RAG-grounded Q&A               |
...
- Free text processed by LLM on save to extract and tag any additional symptoms not captured by cards
...
7. Suggested questions for your provider (LLM-generated, personalized to their patterns)
...
**Data sent to LLM for PDF generation:**
...
Personal data is never sent raw to LLM. The anonymization strategy by context:

| Context                              | What's Sent to LLM                                                       |
| ------------------------------------ | ------------------------------------------------------------------------ |
...
No names, email addresses, exact dates of birth, or location data are ever sent to LLM.
...
- Provider card UI
- Calling assistant (LLM-powered script generation)
...
### Appointment Prep Flow

A guided, multi-step experience that transforms a user's logged data into a structured, provider-ready appointment kit. This is distinct from the existing PDF export — it is interactive, opinionated, and designed around the specific emotional and clinical dynamics of a menopause appointment.

#### Core Problem

Women arrive at menopause appointments with months or years of symptom history, often emotionally activated after prior dismissals, and with 15-20 minutes of provider time. They frequently leave having let the provider set the agenda entirely, having forgotten key concerns, and without data that would have helped them advocate for themselves. Appointment Prep directly addresses this.

#### User Flow

**Step 1 — Set the context**

The user answers three framing questions before anything is generated:

- What kind of appointment is this? (New provider / established relationship)
- What is the primary goal? (Understand where I am / discuss starting HRT / evaluate current treatment / address a specific symptom)
- Have you been dismissed by providers before? (Yes, multiple times / once or twice / no)

These answers shape the tone and content of everything that follows — a first visit with a new specialist requires a different document than a follow-up with an established GP.

**Step 2 — Surface the data story**

Pull the user's last 60-90 days of symptom logs and generate a LLM-written narrative summary of the most significant patterns. This is not a data dump — it is a human-readable story: frequency, co-occurrence, trend direction, and any notable changes. The user can review, edit, and mark which patterns feel most important to them.

**Step 3 — Prioritize concerns**

Present a drag-to-rank interface with the user's top symptoms plus any concerns raised in free-text logs. The user orders them by what matters most to raise in the appointment. This is the most important UX moment in the flow — providers address what is raised first. Helping users lead with what matters most to them, not what comes to mind under stress, is the core value proposition.

**Step 4 — Anticipate the conversation**

LLM generates a set of scenario cards based on the user's goals and symptom profile: "If your provider says X, here is what current research supports and how you might respond." These are not scripts — they are preparation for the moments that typically go sideways. Scenarios are selected based on appointment goal (e.g., if the goal is discussing HRT, common dismissal scenarios around breast cancer risk and WHI are surfaced).
...
| -------------------- | ---------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| Provider Summary     | One-page clinical overview designed to be handed over or shared via patient portal | PDF, clean clinical styling                                                       |
| Personal Cheat Sheet | Prioritized concerns, key questions, conversation anchors                          | PDF or mobile-optimized view, designed for quick reference during the appointment |

#### LLM's Role in This Flow

| Task                                                                | Who Does It |
| ------------------------------------------------------------------- | ----------- |
| Calculate symptom frequency and co-occurrence                       | Python/SQL  |
| Identify trending symptoms (improving/worsening)                    | Python/SQL  |
| Write narrative from calculated patterns                            | LLM         |
| Select relevant dismissal scenarios based on goal + symptom profile | LLM         |
| Generate suggested questions per symptom and goal                   | LLM         |
| Write opening statement draft                                       | LLM         |
| Generate provider summary prose                                     | LLM         |

All data sent to LLM follows the existing anonymization policy: no names, no exact dates of birth, no location data. Symptom patterns are passed as calculated summaries, not raw logs.

#### Privacy Note

The personal cheat sheet may contain more candid language than the provider summary (e.g., notes about prior dismissal experiences, personal priorities). These are kept clearly separate. The provider summary contains only clinical data.

#### Design Principles for This Feature

- **Opinionated, not neutral.** The flow has a point of view: users should lead with what matters most to them, they should know their data, and they should be prepared for common dismissal scenarios. This is not a passive report generator.
- **Editable at every step.** LLM's generated content is always a draft. Users can edit the narrative, reorder priorities, add concerns that didn't appear in logged data, and remove anything that doesn't feel right.
- **Not a replacement for the appointment.** The feature explicitly frames itself as preparation, not a substitute for the provider relationship. Every output includes the standard medical disclaimer.
